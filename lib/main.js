...
function activate() {
  ...
  if (!resolver.available()) {
    atom.notifications.addError("native-icons", {
      description: "The Lumine `atom.app.getFileIcon` service is not available in this build.",
      dismissable: true,
    });
    return;
  }
  ...
  disposables.add(
    ...
    atom.config.observe("native-icons.useCustomFileTypes", () => invalidateEverything()),
    atom.config.observe("native-icons.greenlist", recompile),
    atom.config.observe("native-icons.blacklist", recompile),
  );
}
